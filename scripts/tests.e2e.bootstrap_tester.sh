...
# Run e2e tests for bootstrap tester.

if ! [[ "$0" =~ scripts/tests.e2e.bootstrap_tester.sh ]]; then
  echo "must be run from repository root"
  exit 255
fi

# Determine DIST and ARCH in case installation is required for kubectl or kind
# TODO(marun) Factor this out for reuse (or just switch to nix already)
...
# Deploy a kind cluster with a local registry. Include the local bin in the
# path to ensure locally installed kind and kubectl are available since the
# script expected to call them without a qualifying path.
...
#ginkgo build ./tests/fixture/bootstrap/e2e
#./tests/fixture/bootstrap/e2e/e2e.test --help
